...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.35\linewidth]{figs/Esquema_planta.png}
...
\begin{equation}
J_m \cdot \frac{d\omega_m(t)}{dt} = T_m(t) - B_m \cdot \omega_m(t) - K_{a} \left[ \theta_m(t) - \frac{y_c(t)}{r_p} \right]
\label{ec_motor}
\end{equation}
...
\begin{equation}
J_f \cdot \frac{d\psi(t)}{dt} = -K_f \left[ \delta(t) - \frac{y_c(t)}{r_l} \right] - B_f \cdot \psi(t) - T_f(t) - T_a(t)
\label{ec_rueda}
\end{equation}
...
$I_f$     & Inercia de giro de rueda & $0.270$   & $\mathrm{kg \cdot m^2}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\frac{d\psi(t)}{dt} &= \frac{1}{J_f} \cdot \left\{ -K_f \left[ \delta(t) - \frac{y_c(t)}{r_l} \right] - B_f \cdot \psi(t) - T_f(t) - T_a(t) \right\}
\end{aligned}
\right.
...
La elección de un actuador de $48\ \mathrm{V}$ se considera compatible con arquitecturas automotrices modernas, donde sistemas de $48\ \mathrm{V}$ se utilizan para alimentar cargas de mayor potencia que las disponibles en una red tradicional de $12\ \mathrm{V}$. Esta tendencia se observa, por ejemplo, en sistemas híbiridos como el de \hyperlink{ref:bosch-48v}{Bosch}. En este trabajo se adopta este nivel de tensión para disponer de mayor potencia eléctrica en el actuador. Por este motivo, la tensión efectiva queda limitada por la saturación física del bus de alimentación adoptada:
...
\subsubsection{Efecto de perturbaciones con referencia a \texorpdfstring{$35^\circ$}{35°}}
...
En los gráficos de $F_f$ y $T_f$ se observa que las fricciones de cremallera y pivote de rueda alcanzan sus valores máximos durante el movimiento continuo en una dirección. Una vez alcanzada la referencia, el controlador realiza pequeñas correcciones de tensión para compensar estas perturbaciones y sostener la posición de equilibrio. Aunque las fricciones secas fueron suavizadas mediante funciones arcotangente, siguen comportándose como cargas no lineales alrededor de velocidad nula.

Por otro lado, el torque autoalineante $T_a$ alcanza un valor máximo de $72\ \mathrm{N\,m}$ durante la maniobra. Luego cae prácticamente a cero porque la referencia de $35^\circ$ representa un giro muy grande para una velocidad longitudinal fija de $40\ \mathrm{km/h}$. En el modelo utilizado, al aumentar el ángulo de deriva la fuerza lateral tiende a saturarse y el \textit{pneumatic trail} efectivo disminuye exponencialmente. Por lo tanto, el producto $T_a=F_{y,\mathrm{wheel}}\,t_{\mathrm{eff}}$ presenta un máximo y luego decrece. Esta caída no debe interpretarse como ausencia real de esfuerzo en el neumático, sino como la simplificación adoptada para representar que, a grandes ángulos de deriva, el neumático pierde agarre y el torque autoalineante pierde capacidad restauradora (efecto de subviraje). En este caso en particular, el vehiculo dejaria de rotar por mas que las ruedas hayan rotado.
...
\subsubsection{Efecto de perturbaciones con referencia a \texorpdfstring{$5^\circ$}{5°}}

Para analizar un caso más representativo de una maniobra moderada, se repite la simulación con una referencia trapezoidal de $5^\circ$ y velocidad longitudinal fija de $40\ \mathrm{km/h}$.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{figs/perturbaciones_5deg.png}
    \caption{Perturbaciones $F_f$, $T_f$ y $T_a$ durante la respuesta a referencia trapezoidal de $5^\circ$}
    \label{fig:perturbaciones_5deg}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{figs/rta_5deg.png}
    \caption{Respuesta $\delta(t)$ y $y_c(t)$ ante una referencia trapezoidal de $5^\circ$}
    \label{fig:rta_5deg}
\end{figure}

En este caso el ángulo de deriva permanece dentro de una zona donde el modelo no lleva al neumático a saturación lateral severa. Por ese motivo, la fuerza lateral y el \textit{pneumatic trail} efectivo alcanzan valores estacionarios no nulos cuando $\delta(t)$ se estabiliza. Como ambos términos quedan aproximadamente constantes, el torque autoalineante también converge a un valor constante, en este caso de aproximadamente $56.4\ \mathrm{N\,m}$. Esta perturbación sostenida introduce un error estacionario cercano a $0.6^\circ$ en el seguimiento de la referencia.

No se incorporó acción integral en el controlador final porque, en una aplicación de dirección, la referencia es generada de manera continua por el conductor o por un sistema de asistencia, y no suele permanecer fija durante largos intervalos como en un problema clásico de regulación. Además, en las pruebas realizadas se implementó una variante LQRI, pero no resultó efectiva para eliminar completamente el error estacionario ante perturbaciones sostenidas. Esto se debe a que el integrador fue diseñado sobre el error de $\delta$, mientras que en la arquitectura propuesta la medición disponible para el observador es la posición de cremallera $y_c$. Ante perturbaciones externas aplicadas sobre la rueda, como el torque autoalineante, puede existir una diferencia estacionaria entre $y_c$ y $\delta$ debido a la elasticidad del sistema mecánico.
